...
struct Random {rng: Box<Rng>}

impl Default for Random {
  fn default() -> Random {
    Random {rng: Box::new(thread_rng())}
  }
}
...
mod tests {
    ...
    fn some_random_action() {
        let mut random1 = Random {rng: Box::new(StdRng::from_seed(&[0]))};
        let mut random2 = Random {rng: Box::new(StdRng::from_seed(&[4]))};
